...
fn compatibility() {
    ...
    property(Arbitrary::arbitrary(&mut unstructured).unwrap());
}
        ...
        match (self, other) {
            ...
            (V4(a), V5(b)) => {
                a.field_a == b.field_a
                    ...
                    && matches!(
                        (&a.field_d, &b.field_d),
                        (Some(Enum1::Con1), Some(Enum2::Con1(_))) | (None, None)
                    )
            }
            (V4(a), V6(b)) => {
                a.field_a == b.field_a
                    ...
                    && matches!(
                        (&a.field_d, &b.field_d),
                        (Some(Enum1::Con1), Some(Enum3::Con1(_)))
                            | (None, Some(Enum3::Con2 { .. }))
                            | (None, None)
                    )
            }
...
impl fmt::Debug for Message {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        f.debug_tuple("Message")
            .field(&self.0)
            .field(&const_hex::encode(&self.1))
            .finish()
    }
}

impl Arbitrary<'_> for Message {
    fn arbitrary(g: &mut arbitrary::Unstructured<'_>) -> arbitrary::Result<Self> {
...
impl Arbitrary<'_> for Version1<'static> {
    fn arbitrary(g: &mut arbitrary::Unstructured<'_>) -> arbitrary::Result<Self> {
        Ok(Version1 {
            field_a: rand::random(),
...
impl Arbitrary<'_> for Version2<'static> {
    fn arbitrary(g: &mut arbitrary::Unstructured<'_>) -> arbitrary::Result<Self> {
        Ok(Version2 {
            field_a: rand::random(),
            field_b: if rand::random() {
...
impl Arbitrary<'_> for Version6<'static> {
    fn arbitrary(u: &mut arbitrary::Unstructured<'_>) -> arbitrary::Result<Self> {
        Ok(Version6 {
            field_a: rand::random(),
            field_c: Arbitrary::arbitrary(u)?,
            field_d: Arbitrary::arbitrary(u)?,
        })
    }
}

impl Arbitrary<'_> for Enum1 {
    fn arbitrary(_: &mut arbitrary::Unstructured<'_>) -> arbitrary::Result<Self> {
        Ok(Enum1::Con1)
    }
}

impl Arbitrary<'_> for Enum2 {
    fn arbitrary(g: &mut arbitrary::Unstructured<'_>) -> arbitrary::Result<Self> {
        Ok(Enum2::Con1(Arbitrary::arbitrary(g)?))
    }
}

impl Arbitrary<'_> for Enum3<'static> {
    fn arbitrary(g: &mut arbitrary::Unstructured<'_>) -> arbitrary::Result<Self> {
        if rand::random() {
            Ok(Enum3::Con1(Arbitrary::arbitrary(g)?))
        } else {
            Ok(Enum3::Con2 {
                thing: Arbitrary::arbitrary(g)?,
                bar: if rand::random() {
                    Some(Cow::Owned(Arbitrary::arbitrary(g)?))
                } else {
                    None
