...
fn on_struct(inp: &mut syn::DeriveInput) -> syn::Result<proc_macro2::TokenStream> {
    ...
    let bound = gen_encode_bound(attrs.era().is_some())?;
...
fn gen_encode_bound(has_era: bool) -> syn::Result<syn::TypeParamBound> {
    if has_era {
        syn::parse_str("minicbor::Encode<ledger::protocol::Era>")
    } else {
