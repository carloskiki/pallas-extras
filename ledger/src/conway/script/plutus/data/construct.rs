...
impl Decode<'_> for Construct {
    type Error = tag::Error<collections::Error<fixed::Error<Error>>>;
    ...
    fn decode(d: &mut Decoder<'_>) -> Result<Self, Self::Error> {
        ...
        let (tag, value) = match tag {
            ...
            102 => {
                let mut visitor = d
                    .array_visitor()
                    .map_err(|e| tag::Error::Inner(collections::Error::Malformed(e)))?;
                let tag: u64 = visitor
                    .visit()
                    .ok_or(tag::Error::Inner(collections::Error::Element(
                        collections::fixed::Error::Missing,
                    )))?
