...
pub struct HeaderBody {
    ...
    pub kes_verifying_key: kes::sum::Pow6VerifyingKey<SigningKey, Blake2b256>,
...
impl<C> Encode<C> for HeaderBody {
    fn encode<W: minicbor::encode::Write>(
        &self,
        e: &mut minicbor::Encoder<W>,
        _: &mut C,
    ) -> Result<(), minicbor::encode::Error<W::Error>> {
        e.array(if self.protocol_version.major < protocol::MajorVersion::Vasil { 15 } else { 13 })?;
...
impl<C> Decode<'_, C> for HeaderBody {
    fn decode(d: &mut minicbor::Decoder<'_>, _: &mut C) -> Result<Self, minicbor::decode::Error> {
        ...
        let slot = d.u64()?;
        let previous_hash = d.bytes().map(Some).or_else(|err| {
        ...
        let protocol_less_than_vasil = matches!(d.datatype()?, Type::U8 | Type::U16 | Type::U32 | Type::U64);
        let nonce_vrf: VrfCertificate = if protocol_less_than_vasil {
            // TODO: get the nonce_vrf from the leader_vrf
            VrfCertificate {
                _figure_what_this_is: vec![],
                proof: [0; 80],
            }
        } else {
            d.decode()?
        ...
        let block_body_hash = <[u8; 32] as From<minicbor::bytes::ByteArray<32>>>::from(block_body_hash);
        let kes_verifying_key_bytes: minicbor::bytes::ByteArray<32> = d.decode()?;
        let kes_verifying_key_bytes = <[u8; 32] as From<minicbor::bytes::ByteArray<32>>>::from(kes_verifying_key_bytes);
        let kes_verifying_key_bytes: GenericArray<_, _> = GenericArray::from(kes_verifying_key_bytes);
        let kes_verifying_key = kes::sum::Pow6VerifyingKey::from(kes_verifying_key_bytes);
        let sequence_number = d.u64()?;
        let key_period = d.u8()?;
        let signature = crate::cbor::signature::decode(d, &mut ())?;

        let protocol_version: protocol::Version;
        if protocol_less_than_vasil {
            let major = d.decode()?;
            let minor = d.u8()?;
            protocol_version = protocol::Version { major, minor };
        } else {
            protocol_version = d.decode()?;
        }
        ...
        Ok(HeaderBody {
            ...
            nonce_vrf,
            block_body_size,
            block_body_hash: block_body_hash.into(),
            kes_verifying_key,
            sequence_number,
            key_period,
            signature,
