...
) -> Option<g1::Projective> {
    if scalars.iter().any(|scalar| scalar.signed_bits() > 4096) {
        return None;
    }

    let scalars: Vec<_> = scalars.iter().map(scalar_from_integer).collect();
    Some(bwst::g1::Projective::linear_combination(points, &scalars))
}

pub fn g2_multi_scalar_mul(
    scalars: &[rug::Integer],
    points: &[g2::Projective],
) -> Option<g2::Projective> {
    if scalars.iter().any(|scalar| scalar.signed_bits() > 4096) {
        return None;
    }

    let scalars: Vec<_> = scalars.iter().map(scalar_from_integer).collect();
    Some(bwst::g2::Projective::linear_combination(points, &scalars))
}
